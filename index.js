...
const E = {
  ...
  topService:  "5424818078833715060",
...
// em() — ONLY for message text (parse_mode:"HTML"). Never put in button .text field.
const em = (id, fb) => `<tg-emoji emoji-id="${id}">${fb}</tg-emoji>`;

// ===== JSON DATABASE =====
class DB {
  constructor(path) { this.path = path; this._t = null; this.load(); }
  ...
  load() {
    ...
    // Cleanup invalid channels (remove any that were accidentally added as plain text)
    this._cleanChannels();
    this.save();
  }
  ...
  _cleanChannels() {
    try {
      const raw = JSON.parse(this.data.s.force_chs || "[]");
      // Valid Telegram usernames: 5+ chars, only letters/digits/underscore, no spaces
      const valid = raw.filter(ch =>
        ch && ch.username && /^[a-zA-Z0-9_]{4,}$/.test(ch.username)
      );
      if (valid.length !== raw.length) {
        console.log(`Cleaned ${raw.length - valid.length} invalid channels:`,
          raw.filter(ch => !/^[a-zA-Z0-9_]{4,}$/.test(ch.username || "")).map(c => c.username));
        this.data.s.force_chs = JSON.stringify(valid);
      }
    } catch {}
  }
  ...
  save() {
    ...
    this._t = setTimeout(() => { try { fs.writeFileSync(this.path, JSON.stringify(this.data)); } catch {} }, 400);
  }
  ...
  createUser(id, uname, fname, lname, ref = null) {
    const k = String(id);
    if (this.data.u[k]) {
      if (uname !== undefined && uname) this.data.u[k].uname = uname;
      if (fname) this.data.u[k].fname = fname;
      this.save();
      return false;
    }
    this.data.u[k] = {
      id, uname: uname || null, fname: fname || null, lname: lname || null,
      pts: 0, refs: 0, ref_by: ref, joined: ts(), blocked: 0, svcs: 0, refs_done: []
    };
    this.save();
    return true;
  }
  updateUser(id, p) { const u = this.getUser(id); if (u) { Object.assign(u, p); this.save(); } }
  ...
  addConfig(txt, by) { const c = { id: this.data.cid++, txt, by, used_by: null, used_at: null, at: ts() }; this.data.c.push(c); this.save(); return c; }
  ...
  refDone(rId, nId) { const u = this.getUser(rId); return u && (u.refs_done || []).includes(String(nId)); }
  markRef(rId, nId) { const u = this.getUser(rId); if (u) { if (!u.refs_done) u.refs_done = []; u.refs_done.push(String(nId)); this.save(); } }
}
...
// Force-join confirmation — reply keyboard so we CAN use icon_custom_emoji_id (premium emoji)
function fjConfirmKb() {
  return {
    keyboard: [[{
      text: db.get("forcebtn") || "تأیید عضویت",
...
async function sendFJ(chatId, chs) {
  // Channel links as inline URL buttons
  const btns = chs.map(ch => [{ text: ch.title, url: ch.url }]);
  await bot.sendMessage(chatId, db.get("forcejoin"), {
    parse_mode: "HTML",
    reply_markup: { inline_keyboard: btns }
  });
  // Confirmation as REPLY keyboard — this allows icon_custom_emoji_id (premium emoji)
  await bot.sendMessage(chatId, `${em(E.alarm,"👇")} پس از عضویت دکمه زیر را بزنید:`, {
    parse_mode: "HTML",
    reply_markup: fjConfirmKb(),
  });
}
...
bot.onText(/\/start(.*)/, async (msg, match) => {
  ...
  if (missing.length) return sendFJ(chatId, missing);
...
async function doStart(uid, chatId, from) {
  ...
  // Count referral ONLY ONCE per real joined user
  if (isNew && refBy && !db.refDone(refBy, uid)) {
    const refUser = db.getUser(refBy);
    if (refUser && !refUser.blocked) {
      const pts = parseInt(db.get("ref_pts") || "1");
      const before = refUser.pts;
      db.addPts(refBy, pts, "ref", `دعوت @${from.username || uid}`);
      db.updateUser(refBy, { refs: refUser.refs + 1 });
      db.markRef(refBy, uid);
      try {
        const name = from.username ? `@${from.username}` : (from.first_name || String(uid));
        const txt = db.get("ref_notify")
...
bot.on("callback_query", async (q) => {
  ...
  if (q.data === "go:invite") return showInvite(q.from.id, q.message.chat.id);
});
...
bot.on("message", async (msg) => {
  ...
  // ── GLOBAL: "بازگشت به منوی اصلی" always cancels any state ──
  if (text === "بازگشت به منوی اصلی") {
    clearState(uid);
    if (isAdmin(uid)) {
      return bot.sendMessage(chatId,
        `${em(E.stats,"📊")} <b>پنل مدیریت</b>`,
        { parse_mode:"HTML", reply_markup: adminKb() }
      );
    }
    return bot.sendMessage(chatId, `${em(E.fire,"🔥")} منوی اصلی`, { parse_mode:"HTML", reply_markup: mainKb() });
  }

  // ── Force-join confirmation button (reply keyboard) ──
  const fjBtn = db.get("forcebtn") || "تأیید عضویت";
  if (text === fjBtn && !isAdmin(uid)) {
    const missing = await checkJoin(uid);
    if (missing.length > 0) {
      await bot.sendMessage(chatId,
        `${em(E.alarm,"❌")} هنوز عضو همه کانال‌ها نشدی!`,
        { parse_mode:"HTML" }
      );
      return sendFJ(chatId, missing);
    }
    return doStart(uid, chatId, msg.from);
  }
  ...
  if (!isAdmin(uid)) {
    const user = db.getUser(uid);
    if (user?.blocked) return bot.sendMessage(chatId, db.get("blocked"), { parse_mode:"HTML" });
    // If user doesn't exist yet (somehow got here without /start), check join
    if (!user) {
      const missing = await checkJoin(uid);
      if (missing.length) return sendFJ(chatId, missing);
      return doStart(uid, chatId, msg.from);
  ...
  switch (text) {
    ...
    case "رفع مسدودی":       setState(uid,"UNBLK",{});    return bot.sendMessage(chatId, `${em(E.unblock,"✅")} آیدی عددی کاربر:`,             { parse_mode:"HTML" });
    case "مسدود کردن":       setState(uid,"BLK",{});      return bot.sendMessage(chatId, `${em(E.block,"🚫")} آیدی عددی کاربر:`,              { parse_mode:"HTML" });
    case "افزودن سکه":       setState(uid,"AP_UID",{});   return bot.sendMessage(chatId, `${em(E.addCoin,"💰")} آیدی کاربر:`,                  { parse_mode:"HTML" });
    case "تنظیم سکه":        setState(uid,"SP_UID",{});   return bot.sendMessage(chatId, `${em(E.setCoin,"💲")} آیدی کاربر:`,                  { parse_mode:"HTML" });
    case "ری‌ست سکه":         setState(uid,"RST",{});      return bot.sendMessage(chatId, `${em(E.resetCoin,"🔄")} آیدی کاربر:`,                { parse_mode:"HTML" });
    case "سرویس دستی":       setState(uid,"SVC_UID",{});  return bot.sendMessage(chatId, `${em(E.manualSvc,"🎁")} آیدی کاربر:`,                { parse_mode:"HTML" });
    case "حذف کاربر":        setState(uid,"DEL",{});      return bot.sendMessage(chatId, `${em(E.deleteUser,"🗑")} آیدی کاربر:`,               { parse_mode:"HTML" });
...
async function adminStatus(chatId) {
  return bot.sendMessage(chatId,
    ...
    `${em(E.allUsers,"👥")} کاربران: <b>${db.allUsers().length}</b>\n` +
...
async function adminTopInv(chatId) {
  ...
  return bot.sendMessage(chatId, txt || "خالی", { parse_mode:"HTML" });
}
...
async function adminMonthly(chatId) {
  ...
  const newU = db.allUsers().filter(x=>x.joined?.startsWith(m)).length;
...
async function adminChMenu(uid, chatId) {
  ...
  txt += `\n${em(E.add,"➕")} افزودن: فقط @یوزرنیم یا لینک کانال\n`;
  txt += `${em(E.deleteUser,"➖")} حذف: <code>del @یوزرنیم</code>`;
...
async function adminMsgMenu(uid, chatId) {
  await bot.sendMessage(chatId,
    ...
    `<code>welcome</code> خوش‌آمد | <code>forcejoin</code> عضویت اجباری\n` +
    `<code>forcebtn</code> متن دکمه تأیید | <code>sub_ok</code> موفقیت اشتراک\n` +
    `<code>sub_noconfig</code> کانفیگ نیست | <code>sub_nopoints</code> امتیاز کم\n` +
    `<code>ref_notify</code> اعلام رفرال | <code>rules</code> قوانین\n` +
    `<code>guide</code> راهنما | <code>support</code> پشتیبانی\n` +
    `<code>blocked</code> مسدودی | <code>maintenance</code> تعمیر`,
    { parse_mode:"HTML" }
  );
...
async function adminKbMenu(uid, chatId) {
  ...
  txt += `\n<code>add متن | ردیف | ایموجی‌آیدی</code> — افزودن\n`;
  txt += `<code>del متن‌دکمه</code> — حذف\n`;
  txt += `<code>color متن‌دکمه</code> — رنگی/بی‌رنگ\n`;
  txt += `<code>reset</code> — بازنشانی`;
  await bot.sendMessage(chatId, txt, { parse_mode:"HTML" });
  setState(uid, "KB_CMD", {});
}
async function adminCfgMenu(uid, chatId) {
  ...
  await bot.sendMessage(chatId,
    ...
    `${em(E.block,"🔴")} استفاده‌شده: <b>${db.data.c.filter(c=>c.used_by).length}</b>\n\n` +
    `${em(E.add,"➕")} افزودن: کانفیگ را مستقیم ارسال کنید (هر خط یک کانفیگ)\n` +
    `<code>dellast</code> — حذف آخرین موجود\n` +
    `<code>list</code> — نمایش ۱۰ تا`,
    { parse_mode:"HTML" }
  );
...
async function handleState(uid, chatId, msg, { s, d }) {
  ...
  switch (s) {
    ...
    case "SRCH": {
      ...
      return bot.sendMessage(chatId, fmtUser(u), { parse_mode:"HTML" });
    }
    ...
    case "DEL":   { clearState(uid); const tid=parseInt(text); db.delUser(tid); return bot.sendMessage(chatId,`${em(E.check,"✅")} کاربر <code>${tid}</code> حذف شد.`,{parse_mode:"HTML"}); }
    case "AP_UID": { const tid=parseInt(text); if(isNaN(tid)) return bot.sendMessage(chatId,"❌ آیدی نامعتبر!"); setState(uid,"AP_AMT",{tid}); return bot.sendMessage(chatId,`${em(E.addCoin,"💰")} مقدار امتیاز برای <code>${tid}</code>:`,{parse_mode:"HTML"}); }
    case "AP_AMT": { clearState(uid); const amt=parseInt(text); if(isNaN(amt)) return bot.sendMessage(chatId,"❌ عدد نامعتبر!"); db.addPts(d.tid,amt,"admin","ادمین"); try{await bot.sendMessage(d.tid,`${em(E.addCoin,"💰")} <b>${amt} امتیاز</b> به حسابتان افزوده شد.`,{parse_mode:"HTML"});}catch{} return bot.sendMessage(chatId,`${em(E.check,"✅")} ${amt} امتیاز به <code>${d.tid}</code>.`,{parse_mode:"HTML"}); }
    case "SP_UID": { const tid=parseInt(text); if(isNaN(tid)) return bot.sendMessage(chatId,"❌ آیدی نامعتبر!"); setState(uid,"SP_AMT",{tid}); return bot.sendMessage(chatId,`${em(E.setCoin,"💲")} امتیاز جدید برای <code>${tid}</code>:`,{parse_mode:"HTML"}); }
    case "SP_AMT": { clearState(uid); const amt=parseInt(text); if(isNaN(amt)) return bot.sendMessage(chatId,"❌ عدد نامعتبر!"); db.updateUser(d.tid,{pts:amt}); return bot.sendMessage(chatId,`${em(E.check,"✅")} امتیاز <code>${d.tid}</code> = <b>${amt}</b>`,{parse_mode:"HTML"}); }
    case "RST":   { clearState(uid); const tid=parseInt(text); db.updateUser(tid,{pts:0}); return bot.sendMessage(chatId,`${em(E.check,"✅")} امتیاز <code>${tid}</code> ری‌ست شد.`,{parse_mode:"HTML"}); }
    ...
    case "COIN_CMD": {
      if (text.startsWith("cost ")) { const v=parseInt(text.slice(5)); if(!isNaN(v)){clearState(uid);db.set("cost",String(v));return bot.sendMessage(chatId,`${em(E.check,"✅")} هزینه اشتراک: <b>${v} امتیاز</b>`,{parse_mode:"HTML"});} }
      else if (text.startsWith("ref ")) { const v=parseInt(text.slice(4)); if(!isNaN(v)){clearState(uid);db.set("ref_pts",String(v));return bot.sendMessage(chatId,`${em(E.check,"✅")} امتیاز هر دعوت: <b>${v}</b>`,{parse_mode:"HTML"});} }
      else return bot.sendMessage(chatId,"❌ مثال: <code>cost 2</code> یا <code>ref 1</code>",{parse_mode:"HTML"});
      break;
    }
    case "CH_CMD": {
      ...
      const chs = JSON.parse(db.get("force_chs"));
      if (!chs.find(c=>c.username===uname)) { chs.push({ username: uname, title, url: `https://t.me/${uname}` }); db.set("force_chs", JSON.stringify(chs)); }
      clearState(uid);
      return bot.sendMessage(chatId, `${em(E.check,"✅")} کانال <b>${title}</b> اضافه شد.\n${em(E.alarm,"⚠️")} ربات باید ادمین کانال باشد.`, { parse_mode:"HTML" });
    }
    case "MSG_KEY": {
      const key = text.trim();
      if (!Object.prototype.hasOwnProperty.call(db.data.s, key)) return bot.sendMessage(chatId,"❌ کلید نامعتبر! یکی از کلیدهای نمایش‌داده‌شده را بفرستید.");
      setState(uid, "EDIT_MSG", { key });
      return bot.sendMessage(chatId, `${em(E.edit,"✏️")} متن فعلی:\n${db.get(key)}\n\n<b>متن جدید را ارسال کنید:</b>`, { parse_mode:"HTML" });
    }
    case "EDIT_MSG": { clearState(uid); db.set(d.key, text); return bot.sendMessage(chatId, `${em(E.check,"✅")} پیام <code>${d.key}</code> ذخیره شد.`, { parse_mode:"HTML" }); }
    case "KB_CMD": {
      ...
      if (text.startsWith("add ")) { const pts=text.slice(4).split("|").map(p=>p.trim()); if(pts.length<2) return bot.sendMessage(chatId,"❌ فرمت: add متن | ردیف | ایموجی‌آیدی"); const btnText=pts[0],rowIdx=parseInt(pts[1])-1,eid=pts[2]||null; if(isNaN(rowIdx)||rowIdx<0) return bot.sendMessage(chatId,"❌ ردیف نامعتبر!"); const rows=JSON.parse(db.get("main_kb")); while(rows.length<=rowIdx)rows.push([]); rows[rowIdx].push({text:btnText,...(eid?{eid}:{}),primary:true}); db.set("main_kb",JSON.stringify(rows)); clearState(uid); return bot.sendMessage(chatId,`${em(E.check,"✅")} دکمه «${btnText}» اضافه شد.`,{parse_mode:"HTML"}); }
      if (text.startsWith("del ")) { const btnText=text.slice(4).trim(); const rows=JSON.parse(db.get("main_kb")); let found=false; const nr=rows.map(row=>row.filter(b=>{if(b.text===btnText){found=true;return false;}return true;})).filter(r=>r.length>0); if(!found) return bot.sendMessage(chatId,"❌ دکمه یافت نشد!"); db.set("main_kb",JSON.stringify(nr)); clearState(uid); return bot.sendMessage(chatId,`${em(E.check,"✅")} دکمه «${btnText}» حذف شد.`,{parse_mode:"HTML"}); }
      if (text.startsWith("color ")) { const btnText=text.slice(6).trim(); const rows=JSON.parse(db.get("main_kb")); let found=false; rows.forEach(row=>row.forEach(b=>{if(b.text===btnText){found=true;b.primary=!b.primary;}})); if(!found) return bot.sendMessage(chatId,"❌ دکمه یافت نشد!"); db.set("main_kb",JSON.stringify(rows)); clearState(uid); const np=rows.flat().find(b=>b.text===btnText)?.primary; return bot.sendMessage(chatId,`${em(E.check,"✅")} دکمه «${btnText}» ${np?"🟢 رنگی":"⬜ بی‌رنگ"} شد.`,{parse_mode:"HTML"}); }
      return bot.sendMessage(chatId,"❌ از add / del / color / reset استفاده کنید.",{parse_mode:"HTML"});
    }
    case "CFG_CMD": {
...
async function showInvite(uid, chatId) {
  ...
  const shareText = `با این لینک به ربات کانفیگ رایگان بپیوند و ${pts} امتیاز رایگان بگیر!`;
